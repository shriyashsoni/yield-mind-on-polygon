...
async function pullProtocolData() {
    const portfolio = await fetch("https://api.llama.fi/protocols").then((r) => r.json());
    return {
...
async function generateAllocation() {
    ...
    await db.query(`INSERT INTO ai_recommendations (chain_id, vault_address, recommendation, confidence, model_version)
     VALUES ($1, $2, $3, $4, $5)`, [allocation.chainId, allocation.vault, JSON.stringify(allocation), 0.86, "ensemble-v4"]);
    return allocation;
}
function buildPayloadHash(allocation, nonce, deadline) {
