...
async function logExecution(
  ...
  await db.query(
    ...
    [eventType, txHash, status, JSON.stringify(details)]
  )
}
    ...
    onLogs: async (logs) => {
      for (const log of logs) {
        ...
        if (eventType === "Rebalance") {
          await db.query(
            `INSERT INTO rebalances (tx_hash, rebalance_type, total_assets, metadata)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (tx_hash) DO NOTHING`,
            [
              txHash,
              "autonomous",
              (log.args as { totalAssets?: bigint })?.totalAssets?.toString() ?? "0",
    ...
    onLogs: async (logs) => {
      for (const log of logs) {
        ...
        if (eventType === "VoteCast") {
          ...
          await db.query(
            `INSERT INTO governance_votes (wallet_address, proposal_id, support, weight, tx_hash)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (tx_hash) DO NOTHING`,
            [args.voter, args.proposalId?.toString(), args.support ?? 2, args.weight?.toString() ?? "0", txHash]
          )
        }
      }
    },
